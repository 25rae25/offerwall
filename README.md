...
- **미션 참여 화면** — 일부 캠페인은 버튼 한 번이 아니라 미션 가이드 → 인증코드 입력
  → 적립의 3단계로 진행. 캠페인마다 배경색과 미션 내용이 다르다
...
- **미션형/일반형 캠페인 분기** — 캠페인 타입에 `mission`을 옵셔널 필드로 둬서,
  있으면 미션 화면으로 보내고 없으면 기존 참여 버튼 그대로. 화면 셋(가이드/입력/완료)은
  라우트 하나에서 상태로 전환해 이탈이 곧 미션 중단이 되게 했다.
  정답 대조는 데모라서 클라이언트에 두고 placeholder로 정답을 노출했는데,
  실서비스라면 정답은 서버에만 두고 참여 API가 입력값을 받아 검증해야 한다
